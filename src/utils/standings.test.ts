...
const info: DriverInfo = {
  acronym: new Map([[1, 'VER'], [44, 'HAM'], [4, 'NOR'], [11, 'PER']]),
  fullName: new Map([[1, 'Max Verstappen'], [44, 'Lewis Hamilton'], [4, 'Lando Norris'], [11, 'Sergio Perez']]),
  team: new Map([[1, 'Red Bull'], [44, 'Mercedes'], [4, 'McLaren'], [11, 'Red Bull']]),
  color: new Map([[1, '#1'], [44, '#44'], [4, '#4'], [11, '#11']]),
}
...
const { driverStandings, constructorStandings } = computeStandings(sessions, results, info)
const driver = (n: number) => driverStandings.find((d) => d.driverNumber === n)!
...
describe('computeStandings — constructors', () => {
  ...
  it('ranks teams by points', () => {
    expect(constructorStandings.map((c) => c.name)).toEqual(['Red Bull', 'Mercedes', 'McLaren'])
  })
})
