...
export function Nav() {
  ...
  const selectedMeeting = meetings.data?.find((m) => m.meeting_key === meetingKey)
  ...
  const circuitLabel = selectedMeeting ? selectedMeeting.location.toUpperCase() : null
  const sessionLabel = selectedSession ? selectedSession.session_name.toUpperCase() : null
  const headerLabel =
  ...
  return (
    <header className="shrink-0">
      ...
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1 px-4 py-1.5 bg-track border-b border-panel">
        ...
        <span className="text-[9px] font-bold uppercase tracking-widest text-muted">Year</span>
        <select
        ...
        <span className="text-[9px] font-bold uppercase tracking-widest text-muted">Event</span>
        {meetings.isError && !authFailed ? (
          <span className="text-red-400 font-mono text-[10px]">Failed to load events</span>
        ) : (
          <select
            aria-label="Event"
            value={meetingKey ?? ''}
        ...
        <span className="text-[9px] font-bold uppercase tracking-widest text-muted">Session</span>
        {sessions.isError && !authFailed ? (
          <span className="text-red-400 font-mono text-[10px]">Failed to load sessions</span>
        ) : (
          <select
            aria-label="Session"
            value={sessionKey ?? ''}
