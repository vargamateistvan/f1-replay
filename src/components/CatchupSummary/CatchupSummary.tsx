...
export function CatchupSummary({ summary, drivers, onDismiss }: Props) {
  ...
  if (counts.pit)         headlineParts.push(`${counts.pit} pit stop${counts.pit > 1 ? "s" : ""}`);
  if (counts.flag)        headlineParts.push(`${counts.flag} flag${counts.flag > 1 ? "s" : ""}`);
  if (counts.overtake)    headlineParts.push(`${counts.overtake} overtake${counts.overtake > 1 ? "s" : ""}`);
  if (counts.fastest_lap) headlineParts.push(`${counts.fastest_lap} fastest lap${counts.fastest_lap > 1 ? "s" : ""}`);

  // Pull out the single most notable events for the detail list
  const notable = summary.events.filter(
    (e) => e.kind === "fastest_lap" || e.kind === "flag" || e.kind === "overtake",
  ).slice(0, 5);
    ...
    <div className="absolute bottom-14 left-1/2 -translate-x-1/2 z-40 pointer-events-auto w-[min(340px,90vw)]">
      <div className="bg-[#1f1f27] border border-[#38383f] shadow-2xl overflow-hidden">
        ...
        <div className="flex items-center gap-2 px-3 py-2 bg-[#15151e] border-b border-[#38383f]">
          ...
          <span className="text-[10px] font-mono text-muted ml-1">({fmtDuration(duration)})</span>
          <button
            onClick={onDismiss}
        ...
        {headlineParts.length > 0 && (
          <div className="px-3 py-2 border-b border-[#2a2a35]">
            <p className="text-[11px] text-white/80">{headlineParts.join(" · ")}</p>
          </div>
        )}
        ...
        {notable.length > 0 && (
          <div className="divide-y divide-[#2a2a35]">
            {notable.map((ev) => {
              if (ev.kind === "fastest_lap") {
                ...
                return (
                  <div key={ev.id} className="flex items-center gap-2 px-3 py-1.5">
                    <span className="text-[8px] font-black px-1 py-0.5 bg-[#9b59f5] text-white uppercase tracking-widest shrink-0">FASTEST</span>
                    <span className="text-[11px] font-bold flex-1" style={{ color: "#9b59f5" }}>
                      {d?.name_acronym ?? p.driverNumber}
                    </span>
                    <span className="text-[10px] font-mono tabular-nums" style={{ color: "#9b59f5" }}>
              ...
              if (ev.kind === "flag") {
                const p = ev.payload as { flag: string; message: string; lapNumber: number | null };
                const FLAG_COLORS: Record<string, string> = {
                  RED: "#e8002d", SAFETY_CAR: "#f5a623", VIRTUAL_SC: "#f5a623",
                  YELLOW: "#f5d400", DOUBLE_YELLOW: "#f5d400",
                };
                const color = FLAG_COLORS[p.flag] ?? "#636369";
                return (
                  <div key={ev.id} className="flex items-center gap-2 px-3 py-1.5">
                    <span className="w-[3px] self-stretch shrink-0" style={{ background: color }} />
                    <span className="text-[11px] text-white/80 flex-1 truncate">{p.message}</span>
                  </div>
                );
              }
              if (ev.kind === "overtake") {
                ...
                return (
                  <div key={ev.id} className="flex items-center gap-2 px-3 py-1.5">
                    ...
                    {p.position && (
                      <span className="text-[9px] text-muted ml-auto">P{p.position}</span>
                    )}
                  </div>
                );
              }
