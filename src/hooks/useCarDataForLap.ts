...
export function toTelemetrySamples(
  data: CarData[],
  lapStartMs: number,
): TelemetrySample[] {
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    if (i > 0) {
      ...
      const dtS =
        (new Date(row.date).getTime() - new Date(prev.date).getTime()) / 1000;
...
export function useCarDataForLap(
  ...
  return useQuery({
    ...
    queryFn: () =>
      fetchCarDataForLap(queryClient, sessionKey!, driverNumber!, lapNumber!),
